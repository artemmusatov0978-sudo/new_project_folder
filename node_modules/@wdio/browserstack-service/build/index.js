      ...
      static async uploadEventsData() {
        ...
        try {
          ...
          const ensureEpochTimes = (arr) => {
            const now2 = Date.now();
            const cutoff = 1e12;
            const timeOrigin = typeof performance !== "undefined" && typeof performance.timeOrigin === "number" ? performance.timeOrigin : now2 - process.uptime() * 1e3;
...
var init_testorcherstrationutils = __esm({
  "src/testorchestration/testorcherstrationutils.ts"() {
    ...
    OrchestrationUtils = class _OrchestrationUtils {
...
var FeatureStats, featureStats_default;
var init_featureStats = __esm({
  "src/testOps/featureStats.ts"() {
    "use strict";
    init_bstackLogger();
    init_util();
    FeatureStats = class _FeatureStats {
      triggeredCount = 0;
      sentCount = 0;
      failedCount = 0;
      groups = {};
    ...
    featureStats_default = FeatureStats;
  }
});

// src/testOps/featureUsage.ts
var FeatureUsage, featureUsage_default;
var init_featureUsage = __esm({
  "src/testOps/featureUsage.ts"() {
    "use strict";
    init_util();
    FeatureUsage = class {
    ...
    featureUsage_default = FeatureUsage;
  }
});

// src/testOps/testOpsConfig.ts
var TestOpsConfig, testOpsConfig_default;
var init_testOpsConfig = __esm({
  "src/testOps/testOpsConfig.ts"() {
    "use strict";
    TestOpsConfig = class _TestOpsConfig {
    ...
    testOpsConfig_default = TestOpsConfig;
  }
});

// src/testOps/usageStats.ts
var UsageStats, usageStats_default;
...
var init_accessibility_scripts = __esm({
  "src/scripts/accessibility-scripts.ts"() {
    "use strict";
    AccessibilityScripts = class _AccessibilityScripts {
    ...
    accessibility_scripts_default = AccessibilityScripts.checkAndGetInstance();
  }
});
...
function getCloudProvider(browser) {
  ...
  } else if (browser.options && browser.options.hostname && browser.options.hostname.includes("browserstack")) {
    return "browserstack";
  }
  return "unknown_grid";
}
...
function isMultiRemoteCaps(capabilities) {
  if (!Array.isArray(capabilities)) {
    return true;
  }
...
var init_util = __esm({
  "src/util.ts"() {
    ...
    init_testOpsConfig();
    init_apiUtils();
    init_accessibility_scripts();
    init_fetchWrapper();
    pGitconfig = promisify2(gitconfig);
    ...
    jsonifyAccessibilityArray = (dataArray, keyName, valueName) => {
      const result = {};
    ...
    launchTestSession = PerformanceTester.measureWrapper(TESTHUB_EVENTS.START, o11yErrorHandler(async function launchTestSession2(options, config, bsConfig, bStackConfig, accessibilityAutomation) {
    ...
    isAccessibilityAutomationSession = (accessibilityFlag) => {
      try {
    ...
    isAppAccessibilityAutomationSession = (accessibilityFlag, isAppAutomate) => {
    ...
    _getParamsForAppAccessibility = (commandName) => {
      return {
    ...
    performA11yScan = async (isAppAutomate, browser, isBrowserStackSession, isAccessibility, commandName) => {
    ...
    getAppA11yResults = PerformanceTester.measureWrapper(A11Y_EVENTS.GET_RESULTS, async (isAppAutomate, browser, isBrowserStackSession, isAccessibility, sessionId) => {
    ...
    getAppA11yResultsSummary = PerformanceTester.measureWrapper(A11Y_EVENTS.GET_RESULTS_SUMMARY, async (isAppAutomate, browser, isBrowserStackSession, isAccessibility, sessionId) => {
      if (!isBrowserStackSession) {
        return {};
      }
      if (!isAppAccessibilityAutomationSession(isAccessibility, isAppAutomate)) {
    ...
    getAppA11yResultResponse = async (apiUrl, isAppAutomate, browser, isBrowserStackSession, isAccessibility, sessionId) => {
    ...
    stopBuildUpstream = PerformanceTester.measureWrapper(TESTHUB_EVENTS.STOP, o11yErrorHandler(async function stopBuildUpstream2() {
    ...
    patchConsoleLogs = o11yErrorHandler(() => {
    ...
    getPlatformVersion = o11yErrorHandler(function getPlatformVersion2(caps, userCaps) {
      if (!caps && !userCaps) {
        return void 0;
      }
...
var getProductMap, shouldProcessEventForTesthub, handleErrorForObservability, handleErrorForAccessibility, logBuildError, getProductMapForBuildStartCall;
...
var init_grpcClient = __esm({
  "src/cli/grpcClient.ts"() {
    ...
    GrpcClient = class _GrpcClient {
       ...
       * @returns Worker ID string in format "threadId-processId"
       */
      getClientWorkerIdFromContext(executionContext) {
      ...
      async startBinSession(wdioConfig) {
        ...
        try {
          ...
          request.clientWorkerId = clientWorkerId;
          request.platformIndex = "0";
      ...
      async testSessionEvent(data) {
        PerformanceTester.start(DISPATCHER_EVENTS.TEST_SESSION);
        ...
        try {
          if (!this.client) {
            this.logger.info("No gRPC client not initialized.");
            PerformanceTester.end(DISPATCHER_EVENTS.TEST_SESSION, false, "gRPC client not initialized");
            return;
          }
          ...
          request.clientWorkerId = workerId;
          ...
          try {
            ...
            PerformanceTester.end(DISPATCHER_EVENTS.TEST_SESSION);
            return response;
          } catch (error) {
            const errorMessage = util5.format(error);
            this.logger.error(`testSessionEvent error: ${errorMessage}`);
            PerformanceTester.end(DISPATCHER_EVENTS.TEST_SESSION, false, errorMessage);
            throw error;
          }
        } catch (error) {
          this.logger.error(`Error in TestSessionEvent: ${util5.format(error)}`);
          PerformanceTester.end(DISPATCHER_EVENTS.TEST_SESSION, false, util5.format(error));
          throw error;
        }
      }
      /**
      ...
      async testFrameworkEvent(data) {
        const uniqueEventId = `${DISPATCHER_EVENTS.TEST_FRAMEWORK}-${Date.now()}-${Math.random().toString(36).substring(7)}`;
        ...
        try {
          ...
          try {
            ...
            PerformanceTester.end(uniqueEventId, true, void 0, {
          ...
          PerformanceTester.end(uniqueEventId, false, util5.format(error));
          throw error;
        }
      }
      /**
      ...
      async logCreatedEvent(data) {
        PerformanceTester.start(DISPATCHER_EVENTS.LOG_CREATED);
        this.logger.info("Sending LogCreatedEvent");
        try {
          if (!this.client) {
            this.logger.info("No gRPC client not initialized.");
            PerformanceTester.end(DISPATCHER_EVENTS.LOG_CREATED, false, "gRPC client not initialized");
            return;
          }
          ...
          for (const log5 of logs) {
            const logEntry = LogCreatedEventRequest_LogEntryConstructor.create({
              testFrameworkName: log5.testFrameworkName,
              testFrameworkVersion: log5.testFrameworkVersion,
              testFrameworkState: log5.testFrameworkState,
              uuid: log5.uuid,
              kind: log5.kind,
              message: log5.message,
              timestamp: log5.timestamp,
              level: log5.level
            });
          ...
          try {
            ...
            PerformanceTester.end(DISPATCHER_EVENTS.LOG_CREATED);
            return response;
          } catch (error) {
            const errorMessage = util5.format(error);
            this.logger.error(`logCreatedEvent error: ${errorMessage}`);
            PerformanceTester.end(DISPATCHER_EVENTS.LOG_CREATED, false, errorMessage);
            throw error;
          }
        } catch (error) {
          this.logger.error(`Error in LogCreatedEvent: ${util5.format(error)}`);
          PerformanceTester.end(DISPATCHER_EVENTS.LOG_CREATED, false, util5.format(error));
          throw error;
        }
      }
      async fetchDriverExecuteParamsEvent(data) {
        ...
        try {
          ...
          const platformIndex = data.platformIndex || "0";
          const clientWorkerId = CLIUtils.getClientWorkerId();
          ...
          request.clientWorkerId = clientWorkerId;
          request.platformIndex = platformIndex;
       ...
       */
      async testOrchestrationSession(testFiles, orchestrationStrategy, orchestrationMetadata) {
        try {
          if (!this.client) {
          ...
          const testOrchestrationPromise = promisify3(this.client.testOrchestration).bind(this.client);
          try {
          ...
          this.logger.error(`Error in testOrchestrationSession: ${util5.format(error)}`);
          return null;
        }
      }
      /**
...
var init_eventDispatcher = __esm({
  "src/cli/eventDispatcher.ts"() {
    "use strict";
    EventDispatcher = class _EventDispatcher {
      static #instance = null;
      observers;
      constructor() {
        this.observers = {};
...
var TrackedInstance;
var init_trackedInstance = __esm({
  "src/cli/instances/trackedInstance.ts"() {
    "use strict";
    init_trackedContext();
...
var TestFramework;
var init_testFramework = __esm({
  "src/cli/frameworks/testFramework.ts"() {
    "use strict";
    init_cliUtils();
    init_eventDispatcher();
    init_cliLogger();
    init_trackedInstance();
    TestFramework = class _TestFramework {
...
var TestFrameworkState;
var init_testFrameworkState = __esm({
  "src/cli/states/testFrameworkState.ts"() {
    "use strict";
    TestFrameworkState = Object.freeze({
      /**
...
var HookState;
var init_hookState = __esm({
  "src/cli/states/hookState.ts"() {
    "use strict";
    HookState = Object.freeze({
      /**
...
var init_automationFrameworkConstants = __esm({
  "src/cli/frameworks/constants/automationFrameworkConstants.ts"() {
    "use strict";
...
var AutomationFramework;
var init_automationFramework = __esm({
  "src/cli/frameworks/automationFramework.ts"() {
    "use strict";
    init_cliLogger();
    init_eventDispatcher();
    ...
    AutomationFramework = class _AutomationFramework {
...
var AutomationFrameworkState;
var init_automationFrameworkState = __esm({
  "src/cli/states/automationFrameworkState.ts"() {
    "use strict";
    AutomationFrameworkState = Object.freeze({
      /**
      * Initial state, no session created
      */
      NONE: {
...
var AutomateModule;
var init_automateModule = __esm({
  "src/cli/modules/automateModule.ts"() {
    "use strict";
    init_baseModule();
    init_cliLogger();
    init_testFramework();
    init_testFrameworkState();
    init_hookState();
...
var now, TestFrameworkInstance;
var init_testFrameworkInstance = __esm({
  "src/cli/instances/testFrameworkInstance.ts"() {
    "use strict";
    init_hookState();
    init_testFrameworkState();
    ...
    TestFrameworkInstance = class extends TrackedInstance {
      testFrameworks;
...
var init_wdioMochaTestFramework = __esm({
  "src/cli/frameworks/wdioMochaTestFramework.ts"() {
    ...
    init_util();
    init_constants();
    WdioMochaTestFramework = class _WdioMochaTestFramework extends TestFramework {
...
var AutomationFrameworkInstance;
var init_automationFrameworkInstance = __esm({
  "src/cli/instances/automationFrameworkInstance.ts"() {
    "use strict";
    init_trackedInstance();
    AutomationFrameworkInstance = class extends TrackedInstance {
      frameworkName;
      frameworkVersion;
      state;
...
var WdioAutomationFramework;
var init_wdioAutomationFramework = __esm({
  "src/cli/frameworks/wdioAutomationFramework.ts"() {
    "use strict";
    init_automationFramework();
    init_automationFrameworkState();
    init_cliUtils();
    init_trackedInstance();
    init_automationFrameworkInstance();
    init_cliLogger();
    WdioAutomationFramework = class extends AutomationFramework {
      constructor(automationFrameworkName, automationFrameworkVersion) {
...
var WebdriverIOModule;
var init_webdriverIOModule = __esm({
  "src/cli/modules/webdriverIOModule.ts"() {
    "use strict";
    init_baseModule();
    init_cliLogger();
    ...
    init_util();
    init_grpcClient();
    WebdriverIOModule = class _WebdriverIOModule extends BaseModule {
      name;
      browserName;
      browserVersion;
      platforms;
...
var init_accessibilityModule = __esm({
  "src/cli/modules/accessibilityModule.ts"() {
    "use strict";
    init_baseModule();
    init_cli();
...
var init_observabilityModule = __esm({
  "src/cli/modules/observabilityModule.ts"() {
    "use strict";
    init_baseModule();
    init_automationFramework();
...
var PercyCaptureMap, PercyCaptureMap_default;
var init_PercyCaptureMap = __esm({
  "src/Percy/PercyCaptureMap.ts"() {
    "use strict";
    PercyCaptureMap = class {
      #map = {};
    ...
    PercyCaptureMap_default = PercyCaptureMap;
  }
});

// src/request-handler.ts
var RequestQueueHandler;
var init_request_handler = __esm({
  "src/request-handler.ts"() {
    "use strict";
    init_constants();
    init_bstackLogger();
    RequestQueueHandler = class _RequestQueueHandler {
      queue = [];
      callback;
...
var _TestReporter, TestReporter, reporter_default;
var init_reporter = __esm({
  "src/reporter.ts"() {
    ...
    init_bstackLogger();
    init_listener();
    _TestReporter = class __TestReporter extends WDIOReporter {
      _capabilities = {};
      _config;
      _observability = true;
      _sessionId;
    ...
    TestReporter = o11yClassErrorHandler(_TestReporter);
    reporter_default = TestReporter;
  }
});
...
var init_insights_handler = __esm({
  "src/insights-handler.ts"() {
    ...
    _InsightsHandler = class {
      ...
      async before() {
        PerformanceTester.start(CONFIG_EVENTS.OBSERVABILITY);
        ...
        PerformanceTester.end(CONFIG_EVENTS.OBSERVABILITY);
      }
    ...
    InsightsHandler = o11yClassErrorHandler(_InsightsHandler);
    insights_handler_default = InsightsHandler;
  }
});
...
var require2, tryRequire, percySnapshot, percyAppScreenshot, snapshotHandler, snapshot, screenshotHelper, screenshotHandler, screenshot, screenshotAppHandler, screenshotApp;
var init_PercySDK = __esm({
  "src/Percy/PercySDK.ts"() {
    "use strict";
    ...
    tryRequire = function(pkg, fallback) {
      try {
    ...
    percySnapshot = tryRequire("@percy/selenium-webdriver", null);
    percyAppScreenshot = tryRequire("@percy/appium-app", {});
    snapshotHandler = (...args) => {
    ...
    screenshotHelper = (type3, driverOrName, nameOrOptions, options) => {
      let { name, uuid } = insights_handler_default.currentTest;
      if (isUndefined(name)) {
        ({ name, uuid } = reporter_default.currentTest);
      }
    ...
    screenshotAppHandler = async (...args) => {
      PercyLogger.error("Unsupported driver for percy");
    };
    ...
    screenshotApp = screenshotAppHandler;
  }
});

// src/Percy/Percy-Handler.ts
var _PercyHandler, PercyHandler, Percy_Handler_default;
var init_Percy_Handler = __esm({
  "src/Percy/Percy-Handler.ts"() {
    "use strict";
    init_util();
    init_PercyCaptureMap();
    init_PercySDK();
    init_PercyLogger();
    init_constants();
    init_performance_tester();
    ...
    PercyHandler = o11yClassErrorHandler(_PercyHandler);
    Percy_Handler_default = PercyHandler;
  }
});

// src/cli/modules/percyModule.ts
var PercyModule;
var init_percyModule = __esm({
  "src/cli/modules/percyModule.ts"() {
    "use strict";
    init_baseModule();
    init_cliLogger();
...
var init_cli = __esm({
  "src/cli/index.ts"() {
    ...
    init_testOpsConfig();
    init_wdioMochaTestFramework();
    init_wdioAutomationFramework();
    init_webdriverIOModule();
...
function isValidGitResult(result) {
  return Array.isArray(result.filesChanged) && result.filesChanged.length > 0 && Array.isArray(result.authors) && result.authors.length > 0;
}
  ...
  try {
    try {
      ...
      if (originHeadOutput.startsWith("refs/remotes/origin/")) {
        ...
        log4.debug(`Invalid branch name detected: ${branch}`);
      }
    } catch {
      log4.debug("Could not determine base branch from origin/HEAD");
    }
    try {
      ...
      if (branches.length > 0) {
        ...
        log4.debug(`Invalid branch name detected: ${firstBranch}`);
      }
    } catch {
      log4.debug("Could not determine base branch from local branches");
    }
    try {
      ...
      for (const branch of remoteBranches) {
        ...
        if (cleanBranch.startsWith("origin/") && !cleanBranch.includes("HEAD")) {
          ...
          log4.debug(`Invalid branch name detected: ${cleanBranch}`);
        }
      }
    } catch {
      log4.debug("Could not determine base branch from remote branches");
    }
  } catch (e) {
    log4.debug(`Error finding base branch: ${e}`);
  }
  return null;
}
  ...
  try {
    for (const commit of commitHashes) {
      if (!isValidGitRef(commit)) {
        log4.debug(`Skipping invalid commit hash: ${commit}`);
        continue;
      }
      try {
        ...
        for (const parent of parents) {
          if (!isValidGitRef(parent)) {
            log4.debug(`Skipping invalid parent hash: ${parent}`);
            continue;
          }
        ...
        log4.debug(`Error processing commit ${commit}: ${e}`);
      }
    }
  } catch (e) {
    log4.debug(`Error getting changed files from commits: ${e}`);
  }
...
function getGitMetadataForAISelection(folders = []) {
  ...
  for (const folder of uniqueFolders) {
    ...
    try {
      ...
      if (!isValidGitRef(currentBranch)) {
        log4.warn(`Invalid current branch name detected: ${currentBranch}. Skipping this folder for security reasons.`);
        process.chdir(originalDir);
        continue;
      }
      if (!isValidGitRef(latestCommit)) {
        log4.warn(`Invalid commit hash detected: ${latestCommit}. Skipping this folder for security reasons.`);
        process.chdir(originalDir);
        continue;
      }
      const baseBranch = getBaseBranch();
      log4.debug(`Base branch for comparison: ${baseBranch}`);
      let commits = [];
      if (baseBranch && isValidGitRef(baseBranch)) {
        try {
          const changedFilesOutput = safeGitCommand(["diff", "--name-only", `${baseBranch}..${currentBranch}`]);
          log4.debug(`Changed files between ${baseBranch} and ${currentBranch}: ${changedFilesOutput}`);
          ...
          log4.debug(`Failed to get changed files from branch comparison. Falling back to recent commits. Error: ${error}`);
        ...
        if (baseBranch && !isValidGitRef(baseBranch)) {
          log4.warn(`Invalid base branch name detected: ${baseBranch}. Falling back to recent commits.`);
        }
        const recentCommitsOutput = safeGitCommand(["log", "-10", "--pretty=%H"]);
        commits = recentCommitsOutput.split("\n").filter(Boolean);
        if (commits.length > 0) {
      ...
      if (commits.length > 0) {
        for (const commit of commits) {
          ...
          try {
            ...
            log4.debug(`Processing commit: ${commitMessage}`);
      ...
      if (commits.length === 0 && result.filesChanged.length > 0) {
        try {
          ...
          log4.debug(`Added fallback author: ${fallbackAuthor}`);
        } catch (error) {
          authorsSet.add("Unknown");
          log4.debug(`Added Unknown as fallback author due to error: ${error}`);
        }
      }
      ...
      if ((!result.prTitle || result.prTitle.trim() === "") && latestCommit) {
        try {
          ...
          log4.debug(`Error extracting commit message for PR title: ${e}`);
        }
      }
      process.chdir(originalDir);
      results.push(result);
    } catch (error) {
      log4.error(`Exception in populating Git metadata for AI selection (folder: ${folder}): ${error}`);
      try {
        process.chdir(originalDir);
      } catch (dirError) {
        log4.error(`Error resetting directory: ${dirError}`);
      }
    }
  }
  ...
  const formattedResults = filteredResults.map((result) => ({
    ...
    commitMessages: Array.isArray(result.commitMessages) ? result.commitMessages.map((cm) => ({
      message: cm.message || "",
      user: cm.user || ""
    })) : [],
...
var log4, SAFE_GIT_REF_PATTERN;
var init_helpers = __esm({
  "src/testorchestration/helpers.ts"() {
    "use strict";
    log4 = logger4("wdio-browserstack-service:helpers");
    SAFE_GIT_REF_PATTERN = /^[a-zA-Z0-9_./-]+$/;
  }
});

// src/testorchestration/request-utils.ts
var RequestUtils;
var init_request_utils = __esm({
  "src/testorchestration/request-utils.ts"() {
    "use strict";
    init_bstackLogger();
    init_apiUtils();
    init_fetchWrapper();
    RequestUtils = class _RequestUtils {
      /**
      ...
      static async makeOrchestrationRequest(method, reqEndpoint, options) {
        ...
        const url3 = `${APIUtils.DATA_ENDPOINT}/${reqEndpoint.replace(/^\//, "")}`;
        try {
          const urlObject = new URL(url3);
          if (options.params) {
...
var TestOrderingServer;
var init_test_ordering_server = __esm({
  "src/testorchestration/test-ordering-server.ts"() {
    "use strict";
    init_helpers();
    init_request_utils();
    ...
    TestOrderingServer = class {
      ...
      _processSplitTestsResponse(response) {
        ...
        const normalizeUrl = (url3) => {
          if (!url3) {
            return null;
          }
          return url3.includes(`${APIUtils.DATA_ENDPOINT}/`) ? url3.split(`${APIUtils.DATA_ENDPOINT}/`)[1] : url3;
        };
      ...
      async getOrderedTestFiles() {
        ...
        const timeoutMillis = (this.requestData.timeout || this.defaultTimeout) * 1e3;
...
// src/testorchestration/testorcherstrationhandler.ts
...
var TestOrchestrationHandler;
var init_testorcherstrationhandler = __esm({
  "src/testorchestration/testorcherstrationhandler.ts"() {
    "use strict";
    init_test_ordering_server();
    init_testorcherstrationutils();
    init_grpcClient();
    init_cli();
    init_bstackLogger();
    TestOrchestrationHandler = class _TestOrchestrationHandler {
      static _instance = null;
      config;
       ...
       */
      static getInstance(config) {
        if (_TestOrchestrationHandler._instance === null && config !== null) {
          _TestOrchestrationHandler._instance = new _TestOrchestrationHandler(config);
        }
        return _TestOrchestrationHandler._instance;
      }
      /**
      ...
      _isObservabilityEnabled() {
        return isTrue2(this.config.testObservability);
      }
      /**
       * Checks if test ordering checks should be logged
       */
      shouldLogTestOrderingChecks() {
        return !this.testOrderingEnabled() && this.orchestrationUtils !== null && this.orchestrationUtils.testOrderingEnabled();
      }
      /**
...
// src/testorchestration/apply-orchestration.ts
var apply_orchestration_exports = {};
__export(apply_orchestration_exports, {
  applyOrchestrationIfEnabled: () => applyOrchestrationIfEnabled,
  default: () => apply_orchestration_default
});
...
var apply_orchestration_default;
var init_apply_orchestration = __esm({
  "src/testorchestration/apply-orchestration.ts"() {
    "use strict";
    init_testorcherstrationhandler();
    init_bstackLogger();
    init_util();
    apply_orchestration_default = applyOrchestrationIfEnabled;
  }
});
...
var PercyBinary = class {
  ...
  async download(destParentDir, response) {
    ...
    return new Promise((resolve, reject) => {
      yauzl2.open(zipFilePath, { lazyEntries: true }, function(err, zipfile) {
        if (err) {
          return reject(err);
        }
        zipfile.readEntry();
      ...
      if (cliProcess && cliProcess.pid && cliProcess.exitCode === null) {
        ...
        try {
          if (process.platform === "win32") {
            ...
            BStackLogger2.debug("CLI process terminated successfully with SIGKILL (Unix)");
          }
        } catch (processError2) {
  ...
  async handleHealing(orginalFunc, using, value, browser, options) {
    ...
    try {
      ...
      if (options.selfHeal === true && this.authResult.isHealingEnabled) {
        ...
        if (script) {
          await browser.execute(script);
          PerformanceTester.start(AI_EVENTS.SELF_HEAL_GET_RESULT);
          ...
          PerformanceTester.end(AI_EVENTS.SELF_HEAL_STEP, false, "No healed result found");
        } else {
          PerformanceTester.end(AI_EVENTS.SELF_HEAL_STEP, false, "No healing script generated");
        }
      }
    } catch (err) {
...
var BrowserstackLauncherService = class {
  ...
  async onPrepare(config, capabilities) {
    PerformanceTester.start(FRAMEWORK_EVENTS.INIT);
    ...
    try {
      ...
      if (CLIUtils.checkCLISupportedFrameworks(config.framework) && !isMultiremote) {
        PerformanceTester.start(FRAMEWORK_EVENTS.START);
    ...
    let timer;
    performance4.mark("tbTunnelStart");
    ...
    return Promise.race(
      ...
      clearTimeout(timer);
      performance4.mark("tbTunnelEnd");
      PerformanceTester.end(AUTOMATE_EVENTS.LOCAL_START);
      performance4.measure("bootTime", "tbTunnelStart", "tbTunnelEnd");
  ...
  async onComplete() {
    PerformanceTester.start(EVENTS.SDK_CLEANUP);
    PerformanceTester.start(EVENTS.SDK_ON_STOP);
...
`);
      ...
      try {
        ...
        BStackLogger2.error(`Error while stoping CLI ${err}`);
...
`);
      ...
      PerformanceTester.end(EVENTS.SDK_CLEANUP);
      await PerformanceTester.stopAndGenerate("performance-launcher.html");
    } catch (error) {
...
var _AccessibilityHandler = class {
  ...
  async before(sessionId) {
    PerformanceTester.start(CONFIG_EVENTS.ACCESSIBILITY);
    ...
    accessibility_scripts_default.commandsToWrap.filter((command) => command.name && command.class).forEach((command) => {
      ...
      try {
        const orig = browser[command.name];
...
var BrowserstackService = class {
  ...
  async beforeSession(config, capabilities) {
    PerformanceTester.start(DRIVER_EVENT.INIT);
  ...
  async before(caps, specs, browser) {
    ...
    if (this._browser) {
      try {
        ...
        if (!BrowserstackCLI.getInstance().isRunning()) {
          ...
          this._browser.on("result", (result2) => {
            if (shouldProcessEventForTesthub("")) {
              this._insightsHandler?.browserCommand(
                "client:afterCommand",
                Object.assign(result2, { sessionId }),
                this._currentTest
              );
            }
    ...
    PerformanceTester.end(DRIVER_EVENT.PRE_INITIALIZE);
    PerformanceTester.start(DRIVER_EVENT.POST_INITIALIZE);
    const result = await this._printSessionURL();
    PerformanceTester.end(DRIVER_EVENT.POST_INITIALIZE);
    return result;
  }
  ...
  async after(result) {
    try {
      ...
      PerformanceTester.start(DRIVER_EVENT.QUIT);
      ...
      if (!BrowserstackCLI.getInstance().isRunning()) {
        ...
        try {
          ...
          PerformanceTester.end(EVENTS.SDK_PERCY_TEARDOWN, false, util12.format(error));
        }
      }
      PerformanceTester.start(EVENTS.SDK_WORKER_SAVE_DATA);
      this.saveWorkerData();
      PerformanceTester.end(EVENTS.SDK_WORKER_SAVE_DATA, true);
      PerformanceTester.end(DRIVER_EVENT.QUIT);
